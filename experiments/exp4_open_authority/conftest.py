...
# The PageRank power-iteration lives in exp2_host_graph/analyze.py; put it on sys.path so the
# teleport test can `import analyze` without an inline path hack (avoids ruff E402).
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[1] / "exp2_host_graph"))
